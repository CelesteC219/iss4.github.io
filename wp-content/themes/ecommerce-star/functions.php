...
function ecommerce_star_general_admin_notice(){
         ...
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info" >
				...
				esc_html__('Recommended Actions & Demo Import','ecommerce-star'),
				esc_html__('Use a page builder and Drag widgets (WooCommerce) starting prefix Theme: ex:- Theme:Product slider, Theme:Product categories. !NOTE, If Quick View, Compare, Wishlist litebox not displayed, Enable AJAX Loading from YITH wishlist settings.','ecommerce-star'),	
				esc_html__('Dismiss', 'ecommerce-star'));
		 echo wp_kses_post($msg);
}

if ( isset( $_GET['ecommerce_star_notice_dismissed'] ) ){
	update_option('ecommerce_star_help_notice', 1);
}

$ecommerce_star_help_notice = get_option('ecommerce_star_help_notice', 2);
...
if (($ecommerce_star_help_notice > 1 || $ecommerce_star_help_notice == '') && !$ecommerce_star_refreshed ){
